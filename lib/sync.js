...
const BUFFER_PERIOD_MS = 800 * 1000; // Number of consecutive failed syncs that will lead to a syncState of ERROR as opposed
...
SyncApi.prototype.peek = function (roomId) {
  ...
  return this.client.roomInitialSync(roomId, 20).then(function (response) {
    ...
    const peekRoom = self.createRoom(roomId); // FIXME: Mostly duplicated from _processRoomEvents but not entirely
    ...
    peekRoom.recalculate(); // roll backwards to diverge old state. addEventsToTimeline
...
SyncApi.prototype.sync = function () {
    ...
    if (this.opts.lazyLoadMembers) {
      ...
      if (supported) {
        try {
          ...
          this.opts.filter = await client.createFilter(_filter.Filter.LAZY_LOADING_SYNC_FILTER);
          debuglog("Created and stored lazy load sync filter");
        } catch (err) {
          _logger.logger.error("Creating and storing lazy load sync filter failed", err);
  ...
  async function getFilter() {
    ...
    if (self.opts.filter) {
      ...
      filter = new _filter.Filter(client.credentials.userId);
