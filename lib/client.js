...
function MatrixClient(opts) {
  ...
  this.callList = {// callId: MatrixCall
  ...
  this._pushProcessor = new _pushprocessor.PushProcessor(this); // Cache of the server's /versions response
...
MatrixClient.prototype.initCrypto = async function () {
  ...
  this.reEmitter.reEmit(crypto, ["crypto.keyBackupFailed", "crypto.keyBackupSessionsRemaining", "crypto.roomKeyRequest", "crypto.roomKeyRequestCancellation", "crypto.warning", "crypto.devicesUpdated", "deviceVerificationChanged", "userTrustStatusChanged", "crossSigning.keysChanged"]);
...
wrapCryptoFuncs(MatrixClient, ["resetCrossSigningKeys", "getCrossSigningId", "getStoredCrossSigningForUser", "checkUserTrust", "checkDeviceTrust", "checkOwnCrossSigningTrust", "checkCrossSigningPrivateKey"]);
/**
...
wrapCryptoFuncs(MatrixClient, ["createRecoveryKeyFromPassphrase", "bootstrapSecretStorage", "addSecretStorageKey", "hasSecretStorageKey", "storeSecret", "getSecret", "isSecretStored", "requestSecret", "getDefaultSecretStorageKeyId", "setDefaultSecretStorageKeyId", "checkSecretStoragePrivateKey"]);
/**
...
MatrixClient.prototype.prepareKeyBackupVersion = async function (password, {
  ...
  const [keyInfo, encodedPrivateKey, privateKey] = await this.createRecoveryKeyFromPassphrase(password);
...
MatrixClient.prototype.restoreKeyBackupWithSecretStorage = async function (backupInfo, targetRoomId, targetSessionId) {
  const privKey = (0, olmlib.decodeBase64)((await this.getSecret("m.megolm_backup.v1")));
  return this._restoreKeyBackup(privKey, targetRoomId, targetSessionId, backupInfo);
};
/**
...
MatrixClient.prototype.setRoomMutePushRule = function (scope, roomId, mute) {
  const self = this;
  let deferred, hasDontNotifyRule; // Get the existing room-kind push rule if any
  ...
  if (filterId) {
    ...
    promise = self.getFilter(self.credentials.userId, filterId, true).then(function (existingFilter) {
      ...
      if (error.httpStatus === 404 && (error.errcode === "M_UNKNOWN" || error.errcode === "M_NOT_FOUND")) {
        ...
        self.store.setFilterIdByName(filterName, undefined); // Return a undefined value for existingId further down the promise chain
...
function setupCallEventHandler(client) {
  const candidatesByCall = {// callId: [Candidate]
  ...
  function callEventHandler(event) {
    ...
    let i; //console.info("RECV %s content=%s", event.getType(), JSON.stringify(content));
    ...
    if (event.getType() === "m.call.invite") {
      ...
      if (call) {
        _logger.logger.log("WARN: Already have a MatrixCall with id %s but got an " + "invite. Clobbering.", content.call_id);
      }

      call = (0, _call.createNewMatrixCall)(client, event.getRoomId(), {
      ...
      client.callList[call.callId] = call; // if we stashed candidate events for that call ID, play them back now
...
function checkTurnServers(client) {
  ...
  client.turnServer().then(function (res) {
    ...
    _logger.logger.error("Failed to get TURN URIs");
